...
try {
        ...
        cmhit.push(command)
        mess = {
			wait: 'please wait for loading 🚀',
			success: 'request completed ✅',
			error: {
				stick: 'Cannot access videos⚠️',
				Iv: 'Invalid link🚫',
                api: 'Error🚫'
			},
			only: {
				group: 'Only for with the group⚠️',
				ownerG: 'Only for group owners⚠️',
				ownerB: 'Only for bot owners⚠️',
				admin: 'Only for group admins⚠️',
				Badmin: 'Make the bot a group admin⚠️'
			}
		}
		...
		const katalog = (teks) => {
             res = giie.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© By: Ayogi Akbar_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:fgi})
             giie.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = giie.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6289654560615-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Kw69Oel34Nd0JuluvFNVKt', "groupName": `${NamaBot}`, "footerText": "*_© By: Ayogi Akbar_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:fgi})
...
const fgi = {
	 ...
	 message: { 
                 "videoMessage": { 
                 "title": `${jmn}•${week}, ${calender} ➪ ${pushname}`,
                 "h": `${jmn}•${week}, ${calender} ➪ ${pushname}`,
                 'duration': '1', 
                 'gifPlayback': 'true', 
                 'caption': `${jmn}•${week}, ${calender} ➪ ${pushname}`,
...
giie.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
			...
			ganz = await getBuffer(hooh.result)
			   ...
			   buttons = [{buttonId:`.giie`, buttonText: {displayText: '@giiexsala🥤'}, type: 1},{buttonId:`.status`,buttonText:{displayText:'Info🧁'},type:1},{buttonId:`.command`,buttonText:{displayText:'Feature🍬'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText: '_Developerby: Ayogi Ak_',
               imageMessage: imageMsg,
			   buttons: buttons,
			   headerType: 4
			   }
			   prep = await giie.prepareMessageFromContent(from,{buttonsMessage},{quoted: fgi})
        ...
        const sendWebp = async(from, url) => {
	        ...
	        giie.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:fgi, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
		...
		const simir = (teks) => {
			giie.sendMessage(from, teks, text, { quoted:fgi })
		}
		const math = (teks) => {
...
await giie.updatePresence(from, Presence.composing)
   ...
   const sotoy = [
	    ...
	    if (isCmd && !isGroup) {console.log(color('|CMD|', 'greenyellow'), color(moment(mek.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(`${command} [${args.length}]`, 'cyan'), color(`${pushname}`, 'green'), color(`${sender}`, 'deeppink'))}
	    if (!command) {console.log(color('|MSG|', 'greenyellow'), color(moment(mek.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(cmd, 'cyan'), color(`${pushname}`, 'green'), color(`${sender}`, 'deeppink'))}
...
gionova *X* echa☕︎`
sendButLocation(from, `*Gionova X Echa Bots* \n_If we never try, we will never know, by: ayogi ak_`, `${menu}`, {jpegThumbnail:ganz}, [{buttonId:`.allmenu`,buttonText:{displayText:'All Festure︎'},type:1},{buttonId:`.command`,buttonText:{displayText:'List Menu'},type:1},{buttonId:`.owner`,buttonText:{displayText:'Developer | Creator'},type:1}], {contextInfo: { mentionedJid: [ptod]}})
...
*_Poweredby: Ayogi Ak_*`
giie.sendMessage(from, menu, text, { quoted: fgi, thumbnail: fs.readFileSync('./gio.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://www.instagram.com/p/CUikuHyhvVp/?utm_medium=copy_link`}}})
break
case 'command':
        ptod = "6289654560615@s.whatsapp.net"
        stod = `${sender}`
...
giie.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:fgi})
break
case 'ownermenu':
menu = `*OWNER FEATURE :*
➪ ${prefix}autorespon [ on/off ]
...
➪ ${prefix}delrespon [ nama ]
➪ ${prefix}q [ reply message ]`
punten = [{buttonId: 'command', buttonText: {displayText: 'HOME'}, type: 1},{buttonId: 'creator', buttonText: {displayText: 'DEVELOPER'}, type: 1}]
const mowner = {
    contentText: `${menu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
    headerType: 1
}
await giie.sendMessage(from, mowner, MessageType.buttonsMessage, {quoted: fgi})
break
case 'bugmenu':
menu = `*BUG FEATURE :*
➪ ${prefix}virtex
...
await giie.sendMessage(from, mbug, MessageType.buttonsMessage, {quoted: fgi})
break
case 'settingmenu':
...
await giie.sendMessage(from, msetting, MessageType.buttonsMessage, {quoted: fgi})
break
case 'downloadmenu':
...
await giie.sendMessage(from, mdownload, MessageType.buttonsMessage, {quoted: fgi})
break
case 'makermenu':
...
await giie.sendMessage(from, mmaker1, MessageType.buttonsMessage, {quoted: fgi})
...
await giie.sendMessage(from, mmaker2, MessageType.buttonsMessage, {quoted: fgi})
break
case 'groupmenu':
menu = `*GROUP FEATURE :*
➪ ${prefix}getpict [ @tag ]
...
await giie.sendMessage(from, mgroup, MessageType.buttonsMessage, {quoted: fgi})
break
case 'nsfwmenu':
menu =`*NSFW FEATURE :*
➪ ${prefix}trapnime
...
await giie.sendMessage(from, mnsfw, MessageType.buttonsMessage, {quoted: fgi})
break
case 'othermenu':
menu = `*OTHER FEATURE :*
➪ ${prefix}getscmd
...
await giie.sendMessage(from, mother, MessageType.buttonsMessage, {quoted: fgi})
break
case 'storagemenu':
...
await giie.sendMessage(from, mstorage, MessageType.buttonsMessage, {quoted: fgi})
break
case 'botmenu':
menu = `*CREATE BOT :*
➪ MULAI : untuk jadi bot
➪ STOP : untuk stop jadi bot`
punten = [{buttonId: 'jadibot', buttonText: {displayText: 'MULAI'}, type: 1},{buttonId: 'stopjadibot', buttonText: {displayText: 'STOP'}, type: 1}]
const mbot = {
    contentText: `${menu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
await giie.sendMessage(from, mbot, MessageType.buttonsMessage, {quoted: fgi})
break
...
await giie.sendMessage(from, mfun, MessageType.buttonsMessage, {quoted: fgi})
break
//Hardianto nsfw
case 'yuri':
...
break
case 'giie':
buttonss = [{buttonId: `${prefix}script`, buttonText: {displayText: 'INSTAGRAM🩰'}, type: 1},{buttonId: `${prefix}creator`, buttonText: {displayText: 'WHATSAPP🧸'}, type: 1}]
const bum = {
...
await giie.sendMessage(from, bum, MessageType.buttonsMessage, {quoted: fstick})
break
case 'anal':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
case 'nulis':
               ...
               res = await axios.get(`https://trpfamily.herokuapp.com/api/nulis?text=${nulis}&apikey=gionova`, {json: true})
               if (res.data.error) return reply(res.data.error)
               buff = Buffer.from(res.data.result.split(',')[1], 'base64')
               giie.sendMessage(from, buff, image, {quoted: fgi, caption: mess.success}).catch(e => {
...
case '3dstone': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/3dstone?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText: '_Poweredby: Ayogi Ak_',
			   ...
			   prep = await giie.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
			   giie.relayWAMessage(prep)
			   break
case 'graffiti': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/graffiti?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
...
case 'blackpink': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/blackpink?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'magma': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/magma?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'textpantai': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/textpantai?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'watercolor': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/watercolor?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'jokerlogo': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/jokerlogo?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'thunder': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/thunder?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'breakwall': 
reply(mess.wait)
			  hooh = await fetchJson(`https://janbot-api.herokuapp.com/api/textpro/breakwall?text=${c}`)
			  ganz = await getBuffer(hooh.result)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(ganz, 'imageMessage')).imageMessage
			...
			case 'tolol': 
reply(mess.wait)
			  hooh = await getBuffer(`https://ojanxganz.tk/tolol/img.php?nama=${c}`)
			   menu = `Mission Success.. `
			   buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'BACK'},type:1}]
			   imageMsg = (await giie.prepareMessageMedia(hooh, 'imageMessage')).imageMessage
			   buttonsMessage = {
			   contentText: `${menu}`,
			   footerText: '_Poweredby: Ayogi Ak_',
               imageMessage: imageMsg,
...
break
		...
		case 'sc':
		case 'sourcecode':
		giie.sendMessage(from, { text: "https://instagram.com/giiexsala\n𝗳𝗼𝗹𝗹𝗼𝘄 𝗺𝗲 𝗶𝗻𝘀𝘁𝗮𝗴𝗿𝗮𝗺", matchedText: 'https://instagram.com/giiexsala', description: "", title: "please click here !!!", jpegThumbnail: dfrply2 }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: fgi})
		break
       case 'debug':
...
await giie.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
case 'listgc':
case 'gclist':
case 'listgroup':
...
puntenrespon = [{buttonId: 'autorespon on', buttonText: {displayText: 'ON'}, type: 1},{buttonId: 'autorespon off', buttonText: {displayText: 'OFF'}, type: 1}]
const resp = {
    ...
    headerType: 1
       ...
       if (args.length < 1) return giie.sendMessage(from, resp, MessageType.buttonsMessage, {quoted: fgi})
           if (c === 'on'){
              autorespon = false
...
puntenprefix = [{buttonId: 'setprefix multi', buttonText: {displayText: 'MULTI FREFIX'}, type: 1},{buttonId: 'setprefix nopref', buttonText: {displayText: 'NO PREFIX'}, type: 1}]
const prefx = {
    ...
    headerType: 1
       ...
       if (args.length < 1) return giie.sendMessage(from, prefx, MessageType.buttonsMessage, {quoted: fgi})
           if (c === 'multi'){
              multi = true
...
case 'matrix':
				...
				break
				case 'ttp3d':
				if (!arg) return reply(from, `Penggunaan ${prefix}ttp3d teks`, mek)
				reply(mess.wait)
				sendStickerUrl(from, `https://bx-hunter.herokuapp.com/api/flamingtext/text3d?text=${arg}&apikey=${HunterApi}`)
				break
				case 'trigger':
...
reply(`Succes mematikan autoketik`)
 ...
 anu =`*AUTO KETIK*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan \n*OFF*: untuk menonaktifkan`
punten = [{buttonId: 'autoketik on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'autoketik off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
await giie.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: fgi})
					}
break
...
reply(`Succes mematikan autovn`)
 ...
 anu =`*AUTO VN*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan \n*OFF*: untuk menonaktifkan`
punten = [{buttonId: 'autovn on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'autovn off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
punten = [{buttonId: 'autoread pc', buttonText: {displayText: 'READ PC'}, type: 1},{buttonId: 'autoread gc', buttonText: {displayText: 'READ GC'}, type: 1}]
const autoreads = {
    contentText: `${anu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
anugc =`*AUTO READ GC*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan\n*OFF*: untuk menonaktifkan`
puntengc = [{buttonId: 'autoread gc on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'autoread gc off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const autoreadgc = {
    contentText: `${anugc}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: puntengc,
...
anupc =`*AUTO READ PC*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan\n*OFF*: untuk menonaktifkan `
puntenpc = [{buttonId: 'autoread pc on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'autoread pc off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const autoreadpc = {
    contentText: `${anupc}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: puntenpc,
...
if (args.length < 1) return giie.sendMessage(from, autoreads, MessageType.buttonsMessage, {quoted: fgi})
if (args[0] === "gc") {
if (args.length < 2) return giie.sendMessage(from, autoreadgc, MessageType.buttonsMessage, {quoted: fgi})
if (args[1] === "on") {
...
if (args.length < 2) return giie.sendMessage(from, autoreadpc, MessageType.buttonsMessage, {quoted: fgi})
if (args[1] === "on") {
...
break
 ...
 anu =`*ANTI LINK*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan \n*OFF*: untuk menonaktifkan`
punten = [{buttonId: 'autosticker on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'autosticker off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
await giie.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: fgi})
 ...
 anu =`*NSFW*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan \n*OFF*: untuk menonaktifkan`
punten = [{buttonId: 'nsfw on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'nsfw off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
break
            ...
            punten = [{buttonId: 'slot', buttonText: {displayText: 'SPIN'}, type: 1},{buttonId: 'slotinfo', buttonText: {displayText: 'KETERANGAN'}, type: 1}]
const slott = {
    contentText: `${somtoy}`,
    footerText: '*_Poweredby: Ayogi Ak_*',
    buttons: punten,
...
await giie.sendMessage(from, slott, MessageType.buttonsMessage, {quoted: fgi})
break
            case 'slotinfo':
...
break
            ...
            break
                case 'ttp':  
                    ...
                    giie.sendMessage(from, anu1, image, {quoted: mek, caption : `${prefix}sticker`})
                    break
				case 'attp':
					if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp ${NamaBot}`)
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					giie.sendMessage(from, tagq, text, { quoted: fvid, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					giie.sendMessage(from, tagu, text, { quoted: fvid, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
case 'stats':
     ...
     var stst = await giie.getStatus(`${sender.split('@')[0]}@c.us`)
			...
			var num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
					stamtus = `*INFO SAYA* :
...
break
				...
				giie.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, audio, { quoted: fvid })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await giie.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							giie.sendMessage(_.jid, buff, sticker, { quoted: fvid, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
await giie.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: fgi})
break
					case 'bugpc2':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                ...
                case 'ytmp4':
						...
						try {
							...
							.then((res) => {
								...
								axios.get(`https://trpfamily.herokuapp.com/api/yutub/video?url=${dl_link}&apikey=gionova`)
								.then((a) => {
            ...
            break
					case 'ytmp3':
						...
						try {
							...
							.then((res) => {
								const { dl_link, thumb, title, filesizeF, filesize } = res
								axios.get(`https://trpfamily.herokuapp.com/api/yutub/audio?url=${dl_link}&apikey=gionova`)
								.then((a) => {
				...
				case 'sticker':
					...
					case 's':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							grupinv(`Kirim gambar/video dengan caption ${prefix}sticker atau tag gambar/video yang sudah dikirim\nNote : Durasi video maximal 10 detik`)
						}
						break
